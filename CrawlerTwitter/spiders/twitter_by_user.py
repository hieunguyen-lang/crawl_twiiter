...
class TwitterSpider(scrapy.Spider):
    name = "twitter_by_user"
    handle_httpstatus_list = [403, 404, 200, 401]

    def __init__(self, lang='', maxResults=100, timeStart='', timeGet=5, *args, **kwargs):
        print('Init Spider ...')
        self.lang = lang
    ...
    def start_requests(self):
        ...
        list_data = DispatcherLibrary().get_data_by_user()
        # print list_data
        # list_data = [
        #     (1, '1463811878631530500', 'Topifintech', 1, 38706, 0, 38706, 0, 1),
        #     (2, '1570327895494758405', 'savenowvtm', 1, 38713, 0, 38713, 0, 1),
        #     (3, '1491240914966425600', 'ThinThn94133780', 1, 38411, 0, 38411, 0, 1),
        #     (4, '1543293249791864832', 'KhoongTrang', 1, 38411, 0, 38411, 0, 1)
        # ]

        for row in list_data:
            id, uid, username, brand_id, object_id, service_id, parent_object_id, parent_service_id, status = row
            headers = self.getHeaderRandom()
            print("[INFO] ID: " + str(id) + " -- USERNAME: " + str(username))
            if headers != '':
                url = 'https://api.twitter.com/2/users/'+uid+'/tweets?max_results='+str(self.maxResults)+'&tweet.fields=public_metrics,author_id,created_at,entities,geo,in_reply_to_user_id,lang,possibly_sensitive,referenced_tweets,source'
                yield scrapy.Request(url=url, headers=headers, meta={'headers': headers, 'uid': uid, 'brand_id': brand_id, 'object_id': object_id, 'service_id': service_id, 'parent_object_id': parent_object_id, 'parent_service_id': parent_service_id })
            else:
    ...
    def parse(self, response):
        ...
        if response.status == 403 or response.status == 404:
            ...
            if self.countHeader() > 0:
                headers = self.getHeaderRandom()
                newUrl = 'https://api.twitter.com/2/users/'+uid+'/tweets?max_results='+str(self.maxResults)+'&tweet.fields=public_metrics,author_id,created_at,entities,geo,in_reply_to_user_id,lang,possibly_sensitive,referenced_tweets,source'
            ...
            if "data" in posts:
                ...
                for post in datas:
                    ...
                    if duplicate == True:
                        ...
                        print "[INFO] THE WEB LINK : https://www.twitter.com/"+str(uid)+"/status=" + post_id
                        print ("=======================================")
                        print('\n')
                    else:
                        print('\n')
